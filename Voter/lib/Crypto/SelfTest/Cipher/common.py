    ...
    def _new(self, do_decryption=0):
        ...
        if hasattr(self.module, "MODE_CTR") and self.mode == self.module.MODE_CTR:
            ...
            if not ctr_params.has_key('nbits'):
                ctr_params['nbits'] = 8 * (
                self.module.block_size - len(ctr_params.get('prefix', '')) - len(ctr_params.get('suffix', '')))
...
class CTRSegfaultTest(unittest.TestCase):
    ...
    def shortDescription(self):
        return """Regression test: %s.new(key, %s.MODE_CTR) should raise TypeError, not segfault""" % (
...
class CTRWraparoundTest(unittest.TestCase):
    ...
    def shortDescription(self):
        return """Regression test: %s with MODE_CTR should raise OverflowError on wraparound when shortcut used""" % (
        self.module_name,)
    ...
    def runTest(self):
        ...
        for disable_shortcut in (0, 1):  # (False, True) Test CTR-mode shortcut and PyObject_CallObject code paths
            for little_endian in (0, 1):  # (False, True) Test both endiannesses
                ctr = Counter.new(8 * self.module.block_size, initial_value=2L ** (8 * self.module.block_size) - 1,
                                  little_endian=little_endian, disable_shortcut=disable_shortcut)
...
class CFBSegmentSizeTest(unittest.TestCase):
    ...
    def runTest(self):
        ...
        self.module.new(a2b_hex(self.key), self.module.MODE_CFB, "\0" * self.module.block_size,
                        segment_size=8)  # should succeed
